...
class PortalSession:
    ...
    def __init__(
        ...
        if not registration_number or not password:
            raise VtopLoginError(
                "Registration number and password are required for PortalSession.",
                status_code=400,
            )
    ...
    async def _perform_login_sequence(self) -> LoggedInStudent:
        ...
        for attempt in range(self.max_login_retries):
            print(
                f"PortalSession: Login attempt {attempt + 1}/{self.max_login_retries} for user {self.username[:5]}****"
            )
            try:
                # Step 1: Fetch initial CSRF token
                csrf_token = await fetch_csrf_token(self._client)
                print(
                    f"PortalSession: CSRF TOKEN: {csrf_token[:8]} - **** - **** - ************"
                )
                ...
                print(f"PortalSession: Solved captcha: {captcha_value}")
                ...
                print(f"PortalSession: Login successful for {self.username[:5]}****")
                return logged_in_student

            except VtopCaptchaError as e:
                print(f"PortalSession: Captcha error during login: {e}")
                ...
                print(
                    f"PortalSession: Login failed due to invalid credentials or format: {e}"
                )
                ...
                print(f"PortalSession: Unexpected error during login: {e}")
                if attempt == self.max_login_retries - 1:
                    raise PortalExplorerError(
    ...
    async def _ensure_logged_in(self) -> LoggedInStudent:
        ...
        async with self._login_lock:
            ...
            if self._logged_in_student is None:
                print(
                    f"PortalSession: Not logged in or session expired for {self.username[:5]}****. Initiating login."
                )
            ...
            if (
                ...
                raise PortalExplorerError(
                    "PortalSession: Failed to establish a login session."
                )
            return self._logged_in_student
    ...
    async def close(self):
        """
        Closes the underlying HTTP client. Should be called when done with the PortalSession.
        """
